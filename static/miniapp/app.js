...
function home(){
 ...
 <section class="hero">
  ...
  :`<div class="empty"><span class="big">◈</span><strong>اولین سرویس خودت را فعال کن</strong><span>پلن مناسب را انتخاب کن و در چند قدم کوتاه شروع کن.</span></div><div class="actions"><button class="btn primary" onclick="setTab('services')">مشاهده پلن‌ها</button></div>`}
 </section>
...
function serviceList(list){if(!list?.length)return `<div class="empty"><span class="big">⌁</span><strong>سرویسی ندارید</strong></div>`;return `<div class="service-list">${list.map(s=>`<button class="service-mini" onclick="openService(${s.id})"><div class="service-icon">◈</div><div class="grow"><strong>${esc(s.name)}</strong><small>${statusFa(s.status)} · ${num(s.remaining_days)} روز · ${num(s.remaining_gb)} GB</small></div><span class="chev">‹</span></button>`).join('')}</div>`}
...
function transactions(list){if(!list?.length)return `<div class="empty"><span class="big">₮</span><strong>هنوز تراکنشی ندارید</strong></div>`;return list.map(t=>`<div class="tx"><div class="tx-icon">${t.type==='purchase'?'◈':t.type==='refund'?'↩':'+'}</div><div class="grow"><strong>${esc(t.description||'تراکنش')}</strong><small>${new Date(t.created_at).toLocaleDateString('fa-IR')}</small></div><div class="amount ${Number(t.amount)>=0?'positive':'negative'}">${Number(t.amount)>=0?'+':''}${num(t.amount)}</div></div>`).join('')}
